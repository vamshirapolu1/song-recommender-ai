...
def detect_mood(user_input):
    ...
    is_negated = any(neg + " " in user_input for neg in negations)
    
    mood_scores = {mood: sum(1 for word in keywords if word in user_input) for mood, keywords in mood_keywords.items()}
    best_mood = max(mood_scores, key=mood_scores.get)

    if mood_scores[best_mood] == 0: return "Happy"
    if is_negated:
...
def home():
    ...
    return render_template("index.html", top_songs=top_songs, other_songs=other_songs, detected_mood=detected_mood, selected_language=selected_language)
