...
def detect_mood(user_input):
    ...
    is_negated = any(neg + " " in user_input or user_input.endswith(neg) for neg in negations)
    ...
    # If user says "not happy", suggest "Sad" or "Emotional" instead
    if is_negated and mood_scores[best_mood] > 0:
        if best_mood == "Happy": return "Sad"
        if best_mood == "Chill": return "Energetic"
    
    return best_mood if mood_scores[best_mood] > 0 else "Happy"

@app.route("/", methods=["GET", "POST"])
def home():
    top_songs, other_songs, detected_mood, selected_language = [], [], None, "All"
    ...
    if request.method == "POST":
        ...
        if not filtered.empty:
            ...
            if len(others) > 10: others = others.sample(10)
            other_songs = others.to_dict(orient="records")

    return render_template("index.html", top_songs=top_songs, other_songs=other_songs, 
                           detected_mood=detected_mood, selected_language=selected_language)
